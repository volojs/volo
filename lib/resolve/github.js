...
function resolveGithub(archiveName, fragment, options, callback, errback) {
    ...
    d.resolve(q.call(function () {
        ...
        //Set up URL to repos override for package.json
        var pkgUrl = github.rawUrl('volojs/repos', 'master',
                                        ownerPlusRepo + '/package.json');

        //For speed concerns, check a few URLs to find a valid package.json.
        //This could be wasteful once the majority of projects support
        //package.json with volo properties, so worth a revisit if that
        //condition is met at some point.
        return q.all([
            //Try package.json in the tag
            fetchBrowserPackageJson(ownerPlusRepo, tag),

            //Get the master branch, then fetch its package.json
            github.masterBranch(ownerPlusRepo).then(function (branchName) {
                return fetchBrowserPackageJson(ownerPlusRepo, branchName);
            }),

            //Check repos package override area
            net.getJson(pkgUrl, makeAuthHeaderOptions(pkgUrl))
                .then(null, function () {
                    //Just eat errors, expected that there will be some 404s.
                })
        ]);
    }).then(function (values) {
        var pkg = values[0] || values[1];
        ...
        if (fragment || (browserInfo && browserInfo.url)) {
            ...
            if (fragment) {
                ...
                //the fragment is for a directory.
