...
//If some command line args need IO redirects, use child exec instead of
//spawn. Normally want spawn so that the output can be captured and shown
//to the user (like github ghdeploy commands)
function hasIoRedirect(args) {
        ...
        env: {
            ...
            spawn: function (cmd, args, options) {
                var d = q.defer(),
                    okResponse = '',
                    errResponse = '',
                    spawned, onData, onErrData, origCmd, originalCommand;
                ...
                q.fcall(function () {
                    ...
                    if (hasIoRedirect(args) && options._originalCommand) {
                        //Replace original command string with new one, and
                        //use exec instead. This is a bit of a hack. Ideally
                        //all calls go through exec, but exec needs all the
                        //the same n. and v. processing and listener wiring,
                        //but need to take more time to scrub spawn and exec
                        //use to do a proper cleanup.
                        originalCommand = options._originalCommand;
                        if (origCmd) {
                            originalCommand = originalCommand.replace(new RegExp('^' + origCmd), cmd + ' ' + args[0]);
                        }
                        spawned = exec(originalCommand, options);
                    } else {
                ...
                list.forEach(function (item) {
                    result = result.then(function () {
                        ...
                        if (typeof item === 'string') {
                            options._originalCommand = item;
                            parsed = shellQuote.parse(item);
                        } else {
                            parsed = item;
                        }
                        ...
                        var start = parsed[0],
                            ...
                            args = parsed.splice(useSpawn ? 1 : 2);
