...
var q = require('q'),
    ...
    config = require('./config').get().enterpriseGitHub,
...
/**
 * Sends a request to GitHub Enterprise. Valid options:
 ...
 */
function enterprise(args, opts) {
    opts = opts || {};

    var req,
        options = {},
        localAuth = enterpriseAuth.getLocal(),
        d = q.defer();
    ...
    function retryWithAuth() {
        return enterpriseAuth.fetch().then(function (info) {
            options.token = info.token;
            return enterprise(args, options);
        });
    }
	...
	args.rejectUnauthorized = config.rejectUnauthorized; // ignore cert errors.
    ...
    req = https.request(args, function (response) {
        ...
        response.on('end', function () {
            ...
            if (response.statusCode === 404) {
                ...
                console.log('Enterprise auth required for ' + args.host +
                    args.path + ': ' + body);
...
enterprise.url = function (path) {
    return scheme + '://' + host + '/' + path;
};

enterprise.apiUrl = function (path) {
    return scheme + '://' + apiHost + '/' + path;
};

enterprise.rawUrl = function (ownerPlusRepo, version, specificFile) {
...
enterprise.zipballUrl = function (ownerPlusRepo, version) {
    return enterprise.apiUrl('_nodeload/' + ownerPlusRepo + '/legacy.zip/' + version);
};

enterprise.tags = function (ownerPlusRepo) {
    return enterprise('api/v3/repos/' + ownerPlusRepo + '/tags').then(function (data) {
...
enterprise.repo = function (ownerPlusRepo) {
    return enterprise('api/v3/repos/' + ownerPlusRepo);
};

enterprise.versionTags = function (ownerPlusRepo) {
    return enterprise.tags(ownerPlusRepo).then(function (tagNames) {
        ...
        if (!tagNames.length) {
            return enterprise.masterBranch(ownerPlusRepo).then(function (branchName) {
                return [branchName];
            });
        }
...
enterprise.latestTag = function (ownerRepoVersion) {
    ...
    if (version) {
        ...
        if (semVerRangeRegExp.test(version)) {
            return enterprise.versionTags(ownerRepo).then(function (tagNames) {
            ...
            //to match a real tag.
            return enterprise.tags(ownerRepo).then(function (tags) {
...
enterprise.masterBranch = function (ownerPlusRepo) {
    return enterprise('api/v3/repos/' + ownerPlusRepo).then(function (data) {
        return data.master_branch || 'master';
    });
};

enterprise.search = function (query) {
	return enterprise({
